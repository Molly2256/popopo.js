...
export function mergeEndpoints(
  ...
  return {
    users: {
      ...
      changeOwnerUserId: overrides.users?.changeOwnerUserId ?? defaults.users.changeOwnerUserId,
    },
    spaces: {
    ...
    notifications: {
      collection: overrides.notifications?.collection ?? defaults.notifications.collection,
      byId: overrides.notifications?.byId ?? defaults.notifications.byId,
      markRead: overrides.notifications?.markRead ?? defaults.notifications.markRead,
    },
    ipc: {
      ...
      cancelCurrentSceneRequests:
        overrides.ipc?.cancelCurrentSceneRequests ?? defaults.ipc.cancelCurrentSceneRequests,
      sequencePlayStart: overrides.ipc?.sequencePlayStart ?? defaults.ipc.sequencePlayStart,
      sequencePlayStop: overrides.ipc?.sequencePlayStop ?? defaults.ipc.sequencePlayStop,
      sequenceRecordingStart:
        overrides.ipc?.sequenceRecordingStart ?? defaults.ipc.sequenceRecordingStart,
      sequenceRecordingStop:
        overrides.ipc?.sequenceRecordingStop ?? defaults.ipc.sequenceRecordingStop,
      nameplateNormal: overrides.ipc?.nameplateNormal ?? defaults.ipc.nameplateNormal,
      nameplateSpecial: overrides.ipc?.nameplateSpecial ?? defaults.ipc.nameplateSpecial,
